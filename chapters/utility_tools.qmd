...
- **[uv](https://docs.astral.sh/uv/)**: a Python package manager and virtual-environment tool written in Rust, an order of magnitude faster than pip at resolving and installing. It's a drop-in replacement for `pip`, `pip-tools`, `virtualenv`, and `pyenv`. Create an environment with `uv venv`, install from a `requirements.txt` with `uv pip install -r requirements.txt`, and run project scripts with `uv run`.
