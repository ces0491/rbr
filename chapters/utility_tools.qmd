...
Three more from the same family, for Python tooling specifically:

- **[uv](https://docs.astral.sh/uv/)**: covered properly in [Python and R](languages.qmd) and [Reproducible Environments](environments.qmd), since it's one of the two routes this book takes. Listed here for the parts those chapters don't use: `uv run script.py` executes a script in the project environment without activating anything, and `uv tool install` puts a command-line application in its own environment, the way `pipx` does.
...
Apart from uv, none of these are required to follow this book, and the classic `pip`, `conda`, `grep` and `find` commands all still work. Install them when you get tired of waiting for the originals.
