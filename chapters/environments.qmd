...
You need both. Isolation alone protects you from today's conflicts and not from next year's, and a record of an environment you never isolated just documents a mess.
...
`uv` is a single binary that replaces `pip`, `venv`, and `pyenv`. If you took the uv route in [Python and R](languages.qmd) you already have it and can skip to the next block. Otherwise, install it once:
...
The naming matters more than it looks. The environment you created in [Python and R](languages.qmd) — `datasci` under conda, or the `.venv` in your `rbr-work` folder under uv — is a reasonable scratchpad for working through this book, and it is exactly the pattern this chapter argues against for real work: one shared environment that several projects quietly depend on, so upgrading a package for one analysis breaks another. Give each project its own.
...
For most analysis work, a pinned `requirements.txt` is enough. Prefer a real lockfile for anything whose result someone will act on months later.
...
The rule underneath is that you commit the description of the environment, never the environment itself.
...
- **Let CI catch you.** A continuous integration workflow (covered in [Optimising Workflows](workflows.qmd)) builds your project from nothing but the record file. If you forgot to record something, the build fails — in front of you, on a commit, rather than in front of a colleague six months later. For analysis work this is most of CI's value.
...
That last item is the one people skip. Until you have restored from a record file, you don't know whether it works.

With environments recorded, the foundations are done. The next section turns to producing output someone else can read.
