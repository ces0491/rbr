...
With all the tools and infrastructure in place, let's explore how to optimise your data science workflow for productivity and effectiveness.
...
```bash
# Install the tool (pipx keeps it isolated from your project environments)
...
def test_growth_rate_computes_period_over_period_change():
    ...
    assert result["growth_rate"].tolist() == pytest.approx([None, 0.10, 0.10], nan_ok=True)
...
Run them with `pytest` from the project root. The second test is the more instructive one: division by zero is the sort of edge case that produces `inf` in a spreadsheet, propagates quietly through three more calculations, and surfaces as an absurd figure in a board pack.
...
3. **Model performance tests** that fail the build if accuracy on a held-out set drops below a threshold you set
...
CI tells you something broke after you pushed. Pre-commit hooks tell you before. The [pre-commit](https://pre-commit.com/) framework runs a set of checks against your staged files every time you commit:
...
The honest advice is not to reach for any of them early. A Makefile plus a scheduled GitHub Actions workflow (`on: schedule:` with a cron expression) covers a surprising amount of ground, and none of these tools is free to operate. Adopt one when you have several interdependent pipelines and a real need to know when one of them fails overnight.
...
- **Feature Stores**: Centralised repositories for feature storage and serving
...
None of this is worth doing on a one-off script you'll delete next week. All of it is worth doing on anything a colleague will inherit, anything that runs more than once, and anything whose output someone will act on.
