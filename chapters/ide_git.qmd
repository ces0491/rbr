...
description: >-
  Choosing an editor and learning version control — VS Code, RStudio and JupyterLab, and the Git workflow running underneath all three.
---
...
The practical difference from a plain text editor is that an IDE knows what your code means. It completes function names, tells you a package isn't installed before you run anything, opens documentation without a browser, and puts a terminal and a Git panel next to the file you're editing. On a bad day that's the difference between finding a typo in a second and finding it in a stack trace.
...
- **GitHub Copilot** or **Claude Code**: LLM-assisted code completion and chat, both available as VS Code extensions. Copilot has a free tier for individuals with a monthly completion allowance; Claude Code requires a paid Claude subscription or API credits. The [Utility Tools](utility_tools.qmd) appendix compares the options and the habits worth adopting with them.

Extensions are how VS Code learns a language. Without the Python extension it will happily open a `.py` file and understand nothing about it.
...
4. Choose the environment you made in [Python and R](languages.qmd) — the `.venv` inside your project folder if you took the uv route, or the `datasci` entry if you took conda

VS Code finds `.venv` directories inside the open folder on its own and lists them at the top, so the uv route usually needs one click. Conda environments live centrally and are listed by name.

This tells VS Code which Python installation to run your code with. Get it wrong and you'll be told a package isn't installed when you can see that it is — [Reproducible Environments](environments.qmd) covers why.
...
#### Pointing PyCharm at Your Environment

1. Open PyCharm
2. Create a new project, or open the folder you made in [Python and R](languages.qmd)
3. Click on "Previously configured interpreter"
4. Click the gear icon and select "Add..."

For a **uv environment**, choose "Virtualenv Environment" → "Existing" and point it at `.venv/Scripts/python.exe` inside your project on Windows, or `.venv/bin/python` on macOS and Linux. PyCharm often detects this without being asked.

For a **conda environment**, choose "Conda Environment" → "Existing environment" and browse to its Python:
...
Substitute `anaconda3` for `miniforge3` if you installed Anaconda. When in doubt, activate the environment and run `python -c "import sys; print(sys.executable)"` — that prints the exact path PyCharm wants, whichever kind it is.

:::{.callout-note}
In file paths, forward slashes (/) are used on Linux and macOS, backslashes (\\) on Windows. Python accepts forward slashes on all three, which is the simplest way to sidestep the problem.
:::

### Working with Jupyter Notebooks

Notebooks came up in [Python and R](languages.qmd) as a place to write Python. They are also an editor in their own right, and JupyterLab is the version worth using.
...
```bash
# Install JupyterLab into your activated environment
uv pip install jupyterlab                    # uv
...
JupyterLab gives you several notebooks open at once, a file browser, a terminal, and a data frame viewer, all in one browser tab. Classic Notebook gives you one document.

### Choosing the Right IDE

| Reach for | When |
|---|---|
| **VS Code** | The default. One project mixing Python, R, SQL and notebooks, and you want Git in the same window |
| **RStudio** | R work, especially anything involving renv, Shiny, or knitting a document |
| **JupyterLab** | Exploration, where you want output under each cell and don't yet know what the script will be |
| **PyCharm** | A Python codebase large enough that refactoring and type inference start paying for themselves |

Most people end up with two: one for exploring and one for writing the code that survives. Start with whichever matches the language you use most, and add the second when you notice you want it.
...
Think of Git as a time machine for your code. It saves snapshots of your project at chosen points and lets you go back to any of them.

For analytical work that buys you three things. You can change something without first copying the file to `analysis_v2_final_FINAL.R`, because the old version is recoverable. You can point at the exact state of the code that produced a number, months after you wrote it. And you can hand the project to someone else, or work on it alongside them, without either of you overwriting the other's afternoon.

The rest of the chapter is mechanics. If Git is new to you, the four commands in [Git for Analysts](#git-for-analysts-what-actually-matters) near the end are the ones to learn first.
...
Git runs on your machine and GitHub is where a copy lives that other people — and other machines — can reach. You can use Git without GitHub; you'd have no backup and no way to collaborate.

### Setting Up SSH Authentication for GitHub

SSH keys let you push and pull without typing a password each time, and without a token sitting in a file somewhere:
...
In Windows PowerShell, `eval "$(...)"` is Bash syntax and won't run. Windows ships an SSH agent as a system service instead, which you start once and which then survives reboots — run this in a PowerShell window opened as administrator:
...
Create a repository and run through the core commands:
...
2. Telling Git to start tracking changes in this directory
...
:::{.callout-note}
## PowerShell writes `>` output as UTF-16
...
- **Small reference datasets belong in Git.** A lookup table of country codes is a few kilobytes and versioning it alongside the code is exactly right. Remove the `data/` lines, or narrow them to the directories that actually hold bulk data.
- **Don't ignore file formats wholesale.** Patterns like `*.parquet` or `*.db` are tempting, but [Data Stores](data_stores.qmd) recommends working *from* Parquet — so a blanket rule quietly stops your main working files from ever being tracked. Ignore by location instead of by extension.

If you find yourself wanting to commit something you've already ignored, you can force it with `git add -f <file>`.

If you ever accidentally commit a secret, rotate the credential rather than trying to scrub Git history — [Data Stores](data_stores.qmd) explains why, and what to do.
...
Forking is how you contribute to a repository you don't have write access to, which covers most open-source projects. Within a team you usually have write access, so you skip the fork and push a branch to the shared repository directly.
...
1. **`git add` / `git commit` / `git push`** — the daily loop. Commit when you finish a coherent piece of work, which is rarely the same moment as the end of the day.
...
[Happy Git and GitHub for the useR](https://happygitwithr.com/) covers exactly this situation, and the [References](references.qmd) chapter says more about it.
:::
...
The next chapter covers where your data lives: SQL, the file formats worth knowing, the query engines that have changed what's practical on a laptop, and where credentials belong — which is the first place the `.gitignore` you just wrote starts earning its keep.
