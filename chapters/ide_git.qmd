...
Adjust the patterns to fit your project — this is a starting point, not a rule. Two adjustments come up often:
...
- **Don't ignore file formats wholesale.** Patterns like `*.parquet` or `*.db` are tempting, but [Data Stores](data_stores.qmd) recommends working *from* Parquet — so a blanket rule quietly stops your main working files from ever being tracked. Ignore by location, not by extension.
