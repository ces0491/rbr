...
SQLite is a whole database in a single file, with no server to install or keep running. That makes it the easiest place to learn SQL, and a reasonable choice for real work whenever one program at a time is doing the writing.
...
That creates a table inside `sample.db`, adds three rows, and reads them back.
...
It gives you a spreadsheet-like view of the tables, which is the quickest way to see what's in a database someone hands you.
...
Both languages talk to the same file. You can store data in a database, filter and aggregate it in SQL where that's the clearer way to say it, and pull the result into pandas or a data frame for the modelling.
...
The `.gitignore` template in [Editors and Version Control](ide_git.qmd) already lists both of the files below, under the heading "Secrets: never commit these". This section is why they're in it.
