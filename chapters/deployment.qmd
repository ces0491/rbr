...
ggplot(sales, aes(x = quarter, y = revenue, fill = year)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Quarterly Revenue Comparison")
```
```
...
server <- function(input, output) {
  ...
  output$salesPlot <- renderPlot({
    ggplot(filtered_data(), aes(x = quarter, y = sales, fill = product)) +
      geom_bar(stat = "identity", position = "dodge") +
      theme_minimal() +
      labs(title = paste("Quarterly Sales for", input$year))
  })
