...
Pushing the rendered output by hand gets old quickly. The alternative is to let GitHub render it for you on every push, using GitHub Actions: you commit your source `.qmd` files, and a workflow file at `.github/workflows/publish.yml` describes the machine that should build them. We work through a complete Quarto workflow later in this chapter, under [Deploying a Data Science Report to GitHub Pages](#deploying-a-data-science-report-to-github-pages).
...
**Cost**: Free tier (100GB bandwidth, 300 build minutes per month); paid plans from $19/month
...
If you have read about deploying Shiny apps anywhere else, you have read about **shinyapps.io**. It still works, but Posit is consolidating it into **[Posit Connect Cloud](https://connect.posit.cloud/)** and has published a timeline: a migration tool arrived in Connect Cloud around September 2026, and accounts that haven't migrated are moved automatically from early 2027. Existing URLs get permanent redirects, so links already shared keep working.
...
For apps whose computation is light enough to run in the browser, [**Shinylive**](https://shinylive.io/) compiles an R or Python Shiny app to WebAssembly. The result is static files you can host anywhere — GitHub Pages, Netlify, an S3 bucket — with no server, no cold starts, no hosting bill, and no active-hours limit.
...
App Runner takes a container image or a source repository and runs it as a scaling web service, which puts it in roughly the same category as Cloud Run.
...
Deployment is normally driven from the console or from a `apprunner.yaml` in your repository. If you are not already in the AWS ecosystem, Cloud Run and Render both reach a running container with less ceremony — this is here so the AWS row in the comparison table above has a corresponding section, not because it is the recommended path.
