...
8. **Add a `HEALTHCHECK`** so the runtime can detect and restart broken containers:
   ...
   Healthcheck examples online almost always use `curl`. That fails on the `-slim` images this chapter recommends, because they don't include it — the check errors every time it runs and Docker reports a working container as unhealthy. Use something the image is guaranteed to have.
