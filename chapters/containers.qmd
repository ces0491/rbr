...
The `chown -R` on the mounted `/app` directory is what lets Jupyter save your notebooks back to disk; without it the copied files belong to root and the notebook server can't write to them.
...
1. **Keep images lean**: Smaller images pull faster, deploy faster, and ship less software that could turn out to have a vulnerability in it. Use `-slim` (or `-alpine` for pure-Python workloads) base images when you can.
   ...
   ```dockerfile
   FROM python:3.13-slim  # ~150 MB; the full python:3.13 image is ~1 GB
   ```
...
2. **Use multi-stage builds for production**: Separate building dependencies from runtime
   ...
   The benefit: your final image doesn't carry compilers, header files, or build tools, only the installed Python packages and your application code. For typical data science containers this can cut image size by 30–50%, and every tool you leave out is one that can't be used against you if someone gets into the container.
...
Packaging your code, dependencies, and configuration into a single unit gets you consistent behaviour across machines and makes "it works on mine" a claim you can test rather than argue about.
