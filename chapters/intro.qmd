...
The resources presented in this guide are largely freely available up to some tier (except for the Cloud platforms which are free to setup but incur usage costs), so you can get started without needing to make decisions based on costs.

## Understanding the Command Line

Before starting with specific data science tools, we need to understand one of the most fundamental interfaces in computing: the command line. Many data science tools are best installed, configured, and sometimes even used through this text-based interface.
